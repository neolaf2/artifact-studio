...
1. **One `webview.html` assignment.** Today the `ready` handler calls `refresh()`, which reassigns the HTML — that must stop: `ready` replies with messages only. `refresh()` assigns HTML only the first time. Afterwards the same data goes out as `formHtml` (`schemaFields(schema, data)` + the issues markup) and `rawText`. The document-change handler posts `formHtml` when the change did not originate from a form `edit`, and always posts `rawText`, `saveState`, `status`. Remove all four `previewData` posts, the `buildLivePreviewHtml` require, and its re-export from `module.exports`; delete `artifactPreview.js`.
...
4. **`rawEdit`.** `checkRawText(text, format)`. JSON and ok → `_replaceDocument(document, value)` (the existing `WorkspaceEdit` path) and post `rawState { ok: true }`. Not ok → post the error; **do not touch the document**. YAML (`deferred`) → parse the **text** with the YAML→JSON helper already in `ast.js` *before* touching anything. Parses → replace the document text verbatim (preserving the author's YAML formatting) and post `rawState { ok: true }`. Does not parse → post the error and **do not touch the document**. The Global Constraint holds for both formats.
5. **`compile`.** Post `compileState { running: true }`, clear `compileError`. `await document.save()`. Call the new provider option `onCompile(document, main, recipeFile)` (added beside `onAstSaved`); `recipeFile` is the manifest path the host already resolved via `resolveCompanionPaths`. Success → read the output, post `pdf` + `status`. Failure → `firstDiagnostic(error.message)` → post `compileError`. Always post `compileState { running: false }` in `finally`.
